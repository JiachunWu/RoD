...
def normalize_names(names):
    ...
    got = set(normalized)
    if got != expected:
        raise ValueError(f"ERROR: data.yaml names must contain class ids 0-{NUM_CLASSES - 1}. Missing={sorted(expected - got)}, extra={sorted(got - expected)}.")
...
def validate_artifact_mapping():
    ...
    expected = set(range(NUM_CLASSES))
    if union != expected:
        raise ValueError(f"ERROR: artifact class mapping does not cover all classes 0-{NUM_CLASSES - 1}. Missing={sorted(expected - union)}, extra={sorted(union - expected)}.")
    if inter:
        raise ValueError(f"ERROR: artifact class mapping overlaps for class ids {sorted(inter)}.")


def run_sample_label_tests():
    # Keep the uploaded sample aligned with the v2 sanity-check expectation of 30 boxes.
    example_a = [
        0, 0,
        5, 5, 5,
        0, 0, 0,
    ...
    if label_a != 1 or ids_a != [0, 1, 5] or count_a != 30:
...
def parse_label_file(label_path, strict_labels=False):
    ...
    for line_number, line in enumerate(non_empty, start=1):
        ...
        if class_id < 0 or class_id >= NUM_CLASSES:
            raise ValueError(f"ERROR: label file contains class_id {class_id}, but valid range is 0-{NUM_CLASSES - 1}: {label_path}:{line_number}")
